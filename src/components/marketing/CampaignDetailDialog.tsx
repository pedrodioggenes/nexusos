...
import { RegisterCampaignResultDialog } from './RegisterCampaignResultDialog';
...
export function CampaignDetailDialog({ campaign, open, onOpenChange, onEdit }: CampaignDetailDialogProps) {
  ...
  // Attribution data
  const { data: trackingLinks = [] } = useCampaignTrackingLinks(campaign?.id || '');
  const { data: campaignResults = [] } = useCampaignResults(campaign?.id || '');
  ...
  const getNextStatus = (): { status: string; label: string } | null => {
    switch (campaign.status) {
      case 'approved':
        return { status: 'in_production', label: 'Iniciar Produção' };
      case 'in_production':
        return { status: 'active', label: 'Ativar Campanha' };
      case 'active':
        return { status: 'completed', label: 'Concluir Campanha' };
      default:
        return null;
    }
  };
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="briefing" className="mt-4">
          ...
          <TabsContent value="briefing" className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Card>
                ...
                <CardContent>
                  <p className="font-medium">
                    {formatCurrency(campaign.approved_budget || campaign.planned_budget)}
                  </p>
                  {campaign.approved_budget > 0 && (
                    <p className="text-xs text-muted-foreground">
                      Planejado: {formatCurrency(campaign.planned_budget)}
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            {campaign.briefing && (
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Briefing Detalhado</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-muted-foreground whitespace-pre-wrap">
                    {campaign.briefing}
            ...
            {campaign.supplier && (
              <Card>
                ...
                <CardContent>
                  <p className="font-medium">{campaign.supplier.name}</p>
                </CardContent>
              </Card>
            )}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <Button 
                          ...
                          disabled={rejectCampaign.isPending || !rejectReason.trim()}
                        >
                          Confirmar Rejeição
                        </Button>
                      </div>
            ...
            {nextStatus && (
              <div className="flex justify-end">
                <Button onClick={() => handleStatusChange(nextStatus.status)}>
                  {nextStatus.label}
                </Button>
              </div>
            )}
          ...
          <TabsContent value="metrics" className="space-y-4 mt-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <Card>
                <CardContent className="pt-4">
                  ...
                  <p className="text-2xl font-bold">
                    {(campaign.actual_reach || 0).toLocaleString('pt-BR')}
                  </p>
                  {campaign.expected_reach && (
                    <p className="text-xs text-muted-foreground">
                      Meta: {campaign.expected_reach.toLocaleString('pt-BR')}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="pt-4">
                  ...
                  <p className="text-2xl font-bold">
                    {(campaign.actual_conversions || 0).toLocaleString('pt-BR')}
                  </p>
                  {campaign.expected_conversions && (
                    <p className="text-xs text-muted-foreground">
                      Meta: {campaign.expected_conversions.toLocaleString('pt-BR')}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="pt-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    de {formatCurrency(campaign.approved_budget || campaign.planned_budget)}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="pt-4">
                  ...
                  <p className={cn(
                    'text-2xl font-bold',
                    (campaign.actual_roi || 0) >= 100 ? 'text-green-600' : 'text-red-600'
                  )}>
                    {(campaign.actual_roi || 0).toFixed(1)}%
                  </p>
                  {campaign.expected_roi && (
                    <p className="text-xs text-muted-foreground">
                      Esperado: {campaign.expected_roi.toFixed(1)}%
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">Uso do Orçamento</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="attribution" className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <Card>
                <CardContent className="p-3 text-center">
                  ...
                  <p className="text-[10px] text-muted-foreground">Links UTM</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-3 text-center">
                  <p className="text-2xl font-bold">{campaignResults.length}</p>
              ...
              <Card>
                <CardContent className="p-3 text-center">
                  <p className={cn(
                    'text-2xl font-bold',
                    attributionRoi !== null && attributionRoi >= 0 ? 'text-green-600' : 'text-destructive'
                  )}>
            ...
            {campaignResults.length > 0 && (
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Resultados por Período</CardTitle>
                </CardHeader>
                    ...
                    <div key={r.id} className="p-3 rounded-lg bg-muted/30 space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs">
                        <div>
                          ...
                          <p className="font-medium">{formatCurrency(r.total_investment)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Receita</p>
                          <p className="font-medium">{formatCurrency(r.total_revenue)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">KPIs</p>
                          <p className="font-medium">{Object.keys(r.kpi_snapshot || {}).length} métricas</p>
                        </div>
                      </div>
                      {r.notes && (
            ...
            {trackingLinks.length > 0 && (
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Links de Rastreio</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
          ...
          <TabsContent value="assets" className="space-y-4 mt-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Materiais da Campanha</CardTitle>
              </CardHeader>
              <CardContent>
        ...
        <UTMBuilderDialog
          ...
          open={showUTMBuilder}
          onOpenChange={setShowUTMBuilder}
        />
        <RegisterCampaignResultDialog
          campaignId={campaign.id}
          campaignName={campaign.name}
