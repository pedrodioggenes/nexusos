...
const STORAGE_KEY = "financeiro_sidebar_groups_state";
...
const navigation: NavGroup[] = [
  {
    ...
    items: [
      { title: "Painel Executivo C-Level", icon: LayoutDashboard, path: "/app/financeiro" },
    ],
  },
  {
    title: "DRE Gerencial",
    ...
    items: [
      { title: "DRE Rede", icon: Landmark, path: "/app/financeiro/dre/rede" },
      { title: "DRE Loja", icon: Store, path: "/app/financeiro/dre/loja" },
    ],
  },
  {
    ...
    icon: DollarSign,
    items: [
      { title: "Painel CCC/NCG", icon: RefreshCw, path: "/app/financeiro/capital-giro" },
    ],
  },
  {
    ...
    icon: PieChart,
    items: [
      { title: "Ranking IMC", icon: BarChart3, path: "/app/financeiro/margem-categoria" },
    ],
  },
  {
    title: "Projeção",
    icon: TrendingUp,
    items: [
      { title: "Resultado 30/60/90d", icon: TrendingUp, path: "/app/financeiro/projecao" },
    ],
  },
  {
    ...
    icon: Calculator,
    items: [
      { title: "Análise de Custos", icon: PieChart, path: "/app/financeiro/custos" },
      { title: "Ponto de Equilíbrio", icon: Target, path: "/app/financeiro/ponto-equilibrio" },
    ],
  },
  {
    title: "ROIC",
    icon: RefreshCw,
    items: [
      { title: "Retorno sobre Capital", icon: RefreshCw, path: "/app/financeiro/roic" },
    ],
  },
  {
    title: "Configuração",
    icon: Settings,
    items: [
      { title: "Despesas & Metas", icon: Sliders, path: "/app/financeiro/config/despesas-metas" },
      { title: "Config. Financeira", icon: Settings, path: "/app/financeiro/config/geral" },
    ],
  },
  {
    title: "Suporte",
    icon: HelpCircle,
    items: [
      { title: "Manuais de Uso", icon: BookOpen, path: "/app/financeiro/manuais" },
      { title: "Suporte Técnico", icon: Headphones, path: "/app/financeiro/suporte-tecnico" },
    ],
  },
];
...
export function FinanceiroSidebar({ onNavigate, collapsed = false }: FinanceiroSidebarProps) {
  const location = useLocation();
  const navigate = useNavigate();
  const { userPagesAllowed } = useAuth();

  const filteredNavigation = useMemo(() => {
    if (!userPagesAllowed || userPagesAllowed.financeiro === undefined || userPagesAllowed.financeiro === null) {
      return navigation;
    }
    const allowed = userPagesAllowed.financeiro;
    return navigation
  ...
  const isActive = useCallback((path: string) => {
    if (path === "/app/financeiro") return location.pathname === path;
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          active ? "bg-app-financeiro/10 text-app-financeiro" : "text-muted-foreground hover:bg-muted hover:text-foreground"
                        )}
                      >
                        <item.icon className={cn("h-3.5 w-3.5 shrink-0", active ? "text-app-financeiro" : "text-muted-foreground")} />
              ...
              <div className="space-y-0.5 mt-0.5">
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                       {active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-full bg-app-financeiro" />}
                      <item.icon className={cn("h-3.5 w-3.5 shrink-0 transition-colors", active ? "text-app-financeiro" : "text-muted-foreground group-hover:text-foreground")} />
                      <span className="flex-1 text-left truncate">{item.title}</span>
                    </button>
                  );
                })}
