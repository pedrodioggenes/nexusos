...
const STORAGE_KEY = "cliente_sidebar_groups_state";
...
const navigation: NavGroup[] = [
  {
    ...
    icon: Compass,
    items: [
      { title: "Dashboard", icon: LayoutDashboard, path: "/app/cliente" },
    ],
  },
  {
    title: "Clientes",
    icon: Users,
    items: [
      { title: "Base de Clientes", icon: Users, path: "/app/cliente/clientes" },
      { title: "Segmentação", icon: Star, path: "/app/cliente/segmentacao" },
    ],
  },
  {
    title: "Experiência",
    icon: Heart,
    items: [
      { title: "NPS & Pesquisas", icon: TrendingUp, path: "/app/cliente/nps" },
      { title: "Satisfação", icon: Heart, path: "/app/cliente/satisfacao" },
    ],
  },
  {
    title: "Atendimento",
    icon: Headphones,
    items: [
      { title: "Tickets", icon: MessageSquare, path: "/app/cliente/tickets" },
      { title: "SAC", icon: Headphones, path: "/app/cliente/sac" },
    ],
  },
  {
    title: "Relatórios",
    icon: FileText,
    items: [
      { title: "Relatórios", icon: FileText, path: "/app/cliente/relatorios" },
    ],
  },
  {
    title: "Suporte",
    icon: HelpCircle,
    items: [
      { title: "Manuais de Uso", icon: BookOpen, path: "/app/cliente/manuais" },
      { title: "Suporte Técnico", icon: Headphones, path: "/app/cliente/suporte-tecnico" },
    ],
  },
];
...
export function ClienteSidebar({ onNavigate, collapsed = false }: ClienteSidebarProps) {
  const location = useLocation();
  const navigate = useNavigate();
  const { userPagesAllowed } = useAuth();

  const filteredNavigation = useMemo(() => {
    if (!userPagesAllowed || userPagesAllowed.cliente === undefined || userPagesAllowed.cliente === null) {
      return navigation;
    }
    const allowed = userPagesAllowed.cliente;
    return navigation
  ...
  const isActive = useCallback((path: string) => {
    if (path === "/app/cliente") return location.pathname === path;
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          active ? "bg-app-cliente/10 text-app-cliente" : "text-muted-foreground hover:bg-muted hover:text-foreground"
                        )}
                      >
                        <item.icon className={cn("h-3.5 w-3.5 shrink-0", active ? "text-app-cliente" : "text-muted-foreground")} />
              ...
              <div className="space-y-0.5 mt-0.5">
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                       {active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-full bg-app-cliente" />}
                      <item.icon className={cn("h-3.5 w-3.5 shrink-0 transition-colors", active ? "text-app-cliente" : "text-muted-foreground group-hover:text-foreground")} />
                      <span className="flex-1 text-left truncate">{item.title}</span>
                    </button>
                  );
                })}
