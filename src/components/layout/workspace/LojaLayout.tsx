...
import { ArrowLeft, Store, Menu, ChevronLeft, ChevronRight, Users } from "lucide-react";
...
export function LojaLayout() {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() =>
    localStorage.getItem("loja-sidebar-collapsed") === "true"
  );
  const isMobile = useIsMobile();
  const { canAccessPortal } = useAccessControl();

  useEffect(() => {
    localStorage.setItem("loja-sidebar-collapsed", String(sidebarCollapsed));
  }, [sidebarCollapsed]);
    ...
    <div className="h-[100dvh] w-full flex flex-col overflow-hidden bg-background">
      <header className={cn(
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <SheetContent side="left" className="w-[260px] p-0 border-r border-border bg-card">
                <div className="p-4 border-b border-border">
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-lg bg-app-loja/10 flex items-center justify-center">
                      <Store className="h-4 w-4 text-app-loja" />
                    </div>
                    <span className="font-semibold text-app-loja">Loja</span>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-lg bg-app-loja/10 flex items-center justify-center">
              <Store className="w-4 h-4 text-app-loja" />
            </div>
            <div className="hidden sm:block">
              <h1 className="text-sm font-semibold text-foreground leading-tight">Loja</h1>
              <p className="text-[10px] text-muted-foreground">Inteligência Econômica da Loja</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <NotificationCenter />
          <AppUserMenu user={user} onSignOut={handleSignOut} accentClass="app-loja" />
        </div>
      </header>
