...
import { OfertasSidebar } from "./OfertasSidebar";
import { Button } from "@/components/ui/button";

import { ArrowLeft, Megaphone, Menu, ChevronLeft, ChevronRight, Users } from "lucide-react";
...
/**
 * Ofertas Layout - Clean SaaS Design
 * Inspired by Marketing design system
 */
export function OfertasLayout() {
  const { user, loading, signOut, role, isNosAdmin, isSupplier } = useAuth();
  const navigate = useNavigate();
  const { theme, setTheme } = useTheme();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
    return localStorage.getItem('ofertas-sidebar-collapsed') === 'true';
  });
  ...
  useEffect(() => {
    localStorage.setItem('ofertas-sidebar-collapsed', String(sidebarCollapsed));
  }, [sidebarCollapsed]);
    ...
    <div className="h-[100dvh] w-full flex flex-col overflow-hidden bg-background">
      ...
      <header className={cn(
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              >
                <div className="p-4 border-b border-border">
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-lg bg-app-ofertas/10 flex items-center justify-center">
                      <Megaphone className="h-4 w-4 text-app-ofertas" />
                    </div>
                    <span className="font-semibold text-app-ofertas">Ofertas</span>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-lg bg-app-ofertas/10 flex items-center justify-center">
              <Megaphone className="w-4 h-4 text-app-ofertas" />
            </div>
            <div className="hidden sm:block">
              <h1 className="text-sm font-semibold text-foreground leading-tight">Ofertas</h1>
              <p className="text-[10px] text-muted-foreground">Gestão de Ofertas</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <NotificationCenter />
          <AppUserMenu user={user} onSignOut={handleSignOut} accentClass="app-ofertas" />
        </div>
      </header>
