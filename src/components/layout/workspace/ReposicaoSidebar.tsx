...
const STORAGE_KEY = 'reposicao_sidebar_groups_state';
...
const navigation: NavGroup[] = [
  {
    ...
    icon: Compass,
    items: [
      { title: "Dashboard", icon: LayoutDashboard, path: "/app/reposicao" },
      { title: "Gôndolas", icon: MapPin, path: "/app/reposicao/gondolas" },
      { title: "Tarefas", icon: ClipboardList, path: "/app/reposicao/tarefas" },
    ],
  },
  {
    label: "Controle",
    icon: ShieldCheck,
    items: [
      { title: "Rupturas", icon: AlertTriangle, path: "/app/reposicao/rupturas" },
      { title: "Validades", icon: CalendarClock, path: "/app/reposicao/validades" },
      { title: "Comprovações", icon: Camera, path: "/app/reposicao/comprovacoes" },
    ],
  },
  {
    label: "Operações",
    icon: Layers,
    items: [
      { title: "Roteiros", icon: Route, path: "/app/reposicao/roteiros" },
    ],
  },
  {
    label: "Análise",
    icon: BarChart3,
    items: [
      { title: "Relatórios", icon: BarChart3, path: "/app/reposicao/relatorios" },
    ],
  },
  {
    label: "Suporte",
    icon: HelpCircle,
  items: [
      { title: "Manuais de Uso", icon: BookOpen, path: "/app/reposicao/manuais" },
      { title: "Suporte Técnico", icon: Headphones, path: "/app/reposicao/suporte-tecnico" },
    ],
  },
];
...
interface ReposicaoSidebarProps {
  onNavigate?: () => void;
  onSignOut?: () => void;
  collapsed?: boolean;
  onThemeToggle?: () => void;
...
export function ReposicaoSidebar({ onNavigate, onSignOut, collapsed = false, onThemeToggle, currentTheme }: ReposicaoSidebarProps) {
  const location = useLocation();
  const navigate = useNavigate();
  const { user, signOut } = useAuth();

  const isActive = useCallback((path: string) => {
    if (path === "/app/reposicao") {
    ...
    const buttonContent = (
      <button
        ...
        className={cn(
          ...
          active 
            ? "bg-app-reposicao/10 text-app-reposicao" 
            : "text-muted-foreground hover:bg-muted hover:text-foreground"
        )}
      >
        {active && (
          <div className={cn(
            "absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-full bg-app-reposicao",
            collapsed && "left-0.5"
          )} />
        )}
        ...
        <div className="relative">
          <item.icon className={cn(
            ...
            active ? "text-app-reposicao" : "text-muted-foreground group-hover:text-foreground"
          )} />
