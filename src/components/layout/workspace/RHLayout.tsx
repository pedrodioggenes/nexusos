...
import { ArrowLeft, Users, Menu, ChevronLeft, ChevronRight } from "lucide-react";
...
/**
 * RH Layout - Clean SaaS Design
 * Sapphire Blue theme for HR module
 */
export function RHLayout() {
  ...
  useEffect(() => {
    localStorage.setItem('rh-sidebar-collapsed', String(sidebarCollapsed));
  }, [sidebarCollapsed]);
    ...
    <div className="h-[100dvh] w-full flex flex-col overflow-hidden bg-background">
      ...
      <header className={cn(
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <SheetContent side="left" className="w-[260px] p-0 border-r border-border bg-card">
                <div className="p-4 border-b border-border">
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-lg bg-app-rh/10 flex items-center justify-center">
                      <Users className="h-4 w-4 text-app-rh" />
                    </div>
                    <span className="font-semibold text-app-rh">RH</span>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-lg bg-app-rh/10 flex items-center justify-center">
              <Users className="w-4 h-4 text-app-rh" />
            </div>
            <div className="hidden sm:block">
              <h1 className="text-sm font-semibold text-foreground leading-tight">RH</h1>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <NotificationCenter />
          <AppUserMenu user={user} onSignOut={handleSignOut} accentClass="app-rh" />
        </div>
      </header>
