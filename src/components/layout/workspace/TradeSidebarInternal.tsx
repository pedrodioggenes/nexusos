...
const STORAGE_KEY = 'trade_sidebar_groups_state';
...
const navigation: NavGroup[] = [
  {
    ...
    icon: Compass,
    items: [
      { title: "Dashboard", icon: LayoutDashboard, path: "/app/trade" },
      { title: "Alertas", icon: AlertTriangle, path: "/app/trade/alertas" },
    ],
  },
  {
    label: "Gestão",
    icon: Layers,
    items: [
      { title: "Pacotes", icon: Package, path: "/app/trade/pacotes" },
      { title: "Fornecedores", icon: Building2, path: "/app/trade/fornecedores" },
      { title: "Contratos", icon: FileText, path: "/app/trade/contratos" },
    ],
  },
  {
    label: "Execução",
    icon: ClipboardCheck,
    items: [
      { title: "Checklists", icon: FileCheck, path: "/app/trade/checklists" },
      { title: "Comprovações", icon: Image, path: "/app/trade/comprovacoes" },
      { title: "Calendário", icon: Calendar, path: "/app/trade/calendario" },
    ],
  },
  {
    label: "Análise",
    icon: BarChart3,
    items: [
      { title: "Performance", icon: TrendingUp, path: "/app/trade/performance" },
      { title: "ROI Trade", icon: PieChart, path: "/app/trade/roi" },
      { title: "Relatórios", icon: FileBarChart, path: "/app/trade/relatorios" },
    ],
  },
  {
    label: "Suporte",
    icon: HelpCircle,
  items: [
      { title: "Manuais de Uso", icon: BookOpen, path: "/app/trade/manuais" },
      { title: "Suporte Técnico", icon: Headphones, path: "/app/trade/suporte-tecnico" },
    ],
  },
];
...
interface TradeSidebarInternalProps {
  onNavigate?: () => void;
  onSignOut?: () => void;
  collapsed?: boolean;
  onThemeToggle?: () => void;
...
export function TradeSidebarInternal({ onNavigate, onSignOut, collapsed = false, onThemeToggle, currentTheme }: TradeSidebarInternalProps) {
  const location = useLocation();
  const navigate = useNavigate();
  const { user, signOut, userPagesAllowed } = useAuth();

  const filteredNavigation = useMemo(() => {
    if (!userPagesAllowed || userPagesAllowed.trade === undefined || userPagesAllowed.trade === null) {
      return navigation;
    }
    const allowed = userPagesAllowed.trade;
    return navigation
  ...
  const renderNavItem = (item: NavItem & { badge?: number }) => {
    ...
    const buttonContent = (
      ...
      >
        {active && (
          <div className={cn(
            "absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-full bg-app-trade",
            collapsed && "left-0.5"
          )} />
        )}
        ...
        <div className="relative">
          <item.icon className={cn(
            ...
            active ? "text-app-trade" : "text-muted-foreground group-hover:text-foreground"
          )} />
