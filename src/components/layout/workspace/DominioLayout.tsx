...
import { DominioSidebar } from "./DominioSidebar";
import { Button } from "@/components/ui/button";
import { ArrowLeft, Crown, Menu, ChevronLeft, ChevronRight, Users } from "lucide-react";
...
/**
 * Domínio Layout - Executive Dashboard
 * Amethyst Purple theme for directors/owners
 */
export function DominioLayout() {
  const { user, loading, signOut, role, isNosAdmin, isSupplier } = useAuth();
  const navigate = useNavigate();
  const { theme, setTheme } = useTheme();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
    return localStorage.getItem('dominio-sidebar-collapsed') === 'true';
  });
  ...
  useEffect(() => {
    localStorage.setItem('dominio-sidebar-collapsed', String(sidebarCollapsed));
  }, [sidebarCollapsed]);
    ...
    <div className="h-[100dvh] w-full flex flex-col overflow-hidden bg-background relative">
      ...
      <header className={cn(
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <SheetContent side="left" className="w-[260px] p-0 border-r border-border bg-card">
                <div className="p-4 border-b border-border">
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-lg bg-app-dominio/10 flex items-center justify-center">
                      <Crown className="h-4 w-4 text-app-dominio" />
                    </div>
                    <span className="font-semibold text-app-dominio">Domínio</span>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-lg bg-app-dominio/10 flex items-center justify-center">
              <Crown className="w-4 h-4 text-app-dominio" />
            </div>
            <div className="hidden sm:block">
              <h1 className="text-sm font-semibold text-foreground leading-tight">Domínio</h1>
              <p className="text-[10px] text-muted-foreground">Visão Executiva</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <NotificationCenter />
          <AppUserMenu user={user} onSignOut={handleSignOut} accentClass="app-dominio" />
        </div>
      </header>
