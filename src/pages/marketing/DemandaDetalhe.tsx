...
import { DemandContentViewer } from "@/components/marketing/demands/DemandRichEditor";
import { TransitionReasonDialog } from "@/components/marketing/demands/TransitionReasonDialog";
...
import { ResponseReviewSection } from "@/components/marketing/demands/ResponseReviewSection";
import { WorkflowStepsPanel } from "@/components/marketing/demands/WorkflowStepsPanel";
import { DemandApprovalsPanel } from "@/components/marketing/demands/DemandApprovalsPanel";
import { CompletionChecklist } from "@/components/marketing/demands/CompletionChecklist";
import { DemandDocumentsSection } from "@/components/marketing/demands/DemandDocumentsSection";
import { transitionDemandStatus } from "@/services/demands/transitionDemandStatus";
import { useCompleteDemand } from "@/hooks/useCompletionCriteria";
...
export default function DemandaDetalhe() {
  ...
  const { data: demand, isLoading } = useDemandById(id);
  ...
  if (isLoading) {
    return (
      <PageWrapper
        title="Carregando..."
        icon={<Loader2 className="h-6 w-6 text-app-gestao animate-spin" />}
      >
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <p className="text-muted-foreground mb-4">A demanda solicitada não foi encontrada.</p>
  ...
  const handleTransitionConfirm = async (reason: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        updateDemand.mutate({ id: demand.id, status: reasonDialog.toStatus });
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => navigate(`/app/marketing/demandas/editar/${demand.id}`)}
          >
          ...
          <BlurFade delay={0.1}>
            <div className="space-y-2">
              ...
              {(() => {
                ...
                return <p className="text-sm whitespace-pre-wrap">{demand.description}</p>;
              })()}
            </div>
          </BlurFade>
        )}
        ...
        <BlurFade delay={0.15}>
          <div className="space-y-1 rounded-lg border border-border/50 overflow-hidden">
            ...
            <div className="flex items-center px-3 py-2 hover:bg-muted/30 transition-colors">
              <span className="text-xs text-muted-foreground w-32 shrink-0 flex items-center gap-1.5">
            ...
            {demand.campaign_id && (
              <div className="flex items-center px-3 py-2 hover:bg-muted/30 transition-colors">
                ...
                <Button
                  ...
                  onClick={() => navigate(`/app/marketing/campanhas/${demand.campaign_id}`)}
                >
                  Ver campanha →
                </Button>
              </div>
            ...
            {demand.destination_scope && (
              <div className="flex items-center px-3 py-2 hover:bg-muted/30 transition-colors">
                <span className="text-xs text-muted-foreground w-32 shrink-0">Escopo</span>
                <span className="text-xs">
            ...
            {demand.channels && (demand.channels as string[]).length > 0 && (
              <div className="flex items-center px-3 py-2 hover:bg-muted/30 transition-colors">
                <span className="text-xs text-muted-foreground w-32 shrink-0">Canais</span>
